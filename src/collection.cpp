...
Collection::Collection(MeshData mesh, struct ShaderConfig shader) {
  ...
  DOGL(normal_location = glGetAttribLocation(this->program_id, "normalSmooth"));
  bool init_normals = mesh.normals.has_value() && (normal_location != -1);

  if (mesh.normals.has_value() && (normal_location == -1)) {
    std::cerr << "[C++][WARN] Normals provided but normal input not found in "
                 "vertex shader."
              << std::endl;
  }
  ...
  if (init_normals) {
    ...
    DOGL(glVertexAttribPointer(normal_location, 3, GL_FLOAT, GL_FALSE, 0, 0));
